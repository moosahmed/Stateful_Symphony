resource "aws_key_pair" "kubernetes" {
  key_name   = "${terraform.workspace}-pub_key"
  public_key = "${file("${path.root}/data/aws_key_pair_public_key")}"
}

resource "aws_iam_role" "master" {
  name               = "${terraform.workspace}-masters.${var.k8s_cluster}"
  assume_role_policy = "${file("${path.root}/data/aws_iam_role_masters_policy")}"
}

resource "aws_iam_role" "node" {
  name               = "${terraform.workspace}-nodes.${var.k8s_cluster}"
...
resource "aws_iam_group" "iam-group" {
  name = "${terraform.workspace}-iam-group"
}
